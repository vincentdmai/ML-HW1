...
def cross_validate(trainer, predictor, all_data, all_labels, folds, params):
    ...
    model = trainer(all_data, all_labels, params)

    sIndex = 0
    for i in range(folds):

        label = predictor(all_data[:, sIndex:sIndex+examples_per_fold], model)
        accuracy = np.sum(label == all_labels[sIndex:sIndex+examples_per_fold])
        accuracy /= examples_per_fold
        sIndex+=examples_per_fold
        models.append(label)
        scores[i] = accuracy
